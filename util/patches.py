...
def OpenDataFiles2(train_path, run_60, Test = False):
    # Initialize in able to concatenate
    data20_gt = data60_gt = data10 = data20 = data60 = None
    # train = label = None
    data10 = np.load(train_path + '/data10.npy')
    data20 = np.load(train_path + '/data20.npy')

    if run_60:
        data60_gt = np.load(train_path + '/data60_gt.npy')
        data60 = np.load(train_path + '/data60.npy')
        return Split_Tra_Val_Tes(train_path, [data10, data20, data60], data60_gt,run_60, Test)
    else:
        data20_gt = np.load(train_path + '/data20_gt.npy')
        return Split_Tra_Val_Tes(train_path, [data10, data20], data20_gt, run_60, Test)
def OpenDataFilesLandsat(train_path, Test = False):
    # Initialize in able to concatenate
    data30_gt = data10 = data15 = data30 = None
    # train = label = None
    data10 = np.load(train_path + '/data10.npy')
    data15 = np.load(train_path + '/data15.npy')
    data30 = np.load(train_path + '/data30.npy')

    data30_gt = np.load(train_path + '/data30_gt.npy')
    return Split_Tra_Val_Tes_LANDSAT(train_path, [data10, data15, data30], data30_gt, Test)

def Split_Tra_Val_Tes(train_path, train, label, run_60 = True, Test = False):
    try:
        train_ind = np.load(train_path + '/training_index.npy')
        val_ind = np.load(train_path + '/validation_index.npy')
    ...
    if Test:    
        test_ind = np.load(train_path + '/testing_index.npy')
        test_data = [p[test_ind] for p in train]   
        test_label = label[test_ind]
        print("Loaded {} patches for testing.".format(test_ind.shape[0]))
        if run_60:
            test_data = [test_data[0].transpose(0, 2, 3, 1), test_data[1].transpose(0, 2, 3, 1), test_data[2].transpose(0, 2, 3, 1)]
        else:       
            test_data = [test_data[0].transpose(0, 2, 3, 1), test_data[1].transpose(0, 2, 3, 1)]
        test_label = test_label.transpose(0, 2, 3, 1)
        return test_data, test_label
    else:
        train_data = [p[train_ind] for p in train]    
        train_label = label[train_ind]
        
        val_data = [p[val_ind] for p in train]
        val_label = label[val_ind]
        print("Loaded {} patches for training.".format(train_ind.shape[0]))
        print("Loaded {} patches for validation.".format(val_ind.shape[0]))
        
        if run_60:
            train_data = [train_data[0].transpose(0, 2, 3, 1), train_data[1].transpose(0, 2, 3, 1), train_data[2].transpose(0, 2, 3, 1)]       
            val_data = [val_data[0].transpose(0, 2, 3, 1), val_data[1].transpose(0, 2, 3, 1), val_data[2].transpose(0, 2, 3, 1)]
        else:
            train_data = [train_data[0].transpose(0, 2, 3, 1), train_data[1].transpose(0, 2, 3, 1)]                 
            val_data = [val_data[0].transpose(0, 2, 3, 1), val_data[1].transpose(0, 2, 3, 1)]
        train_label = train_label.transpose(0, 2, 3, 1)
        val_label = val_label.transpose(0, 2, 3, 1)
        return train_data, train_label, val_data, val_label
def Split_Tra_Val_Tes_LANDSAT(train_path, train, label, Test = False):
...
def patches_equa(img):
    [m, n, band] = img.shape
 
    img_equa= np.zeros((m, n, band)).astype(np.float32)
    img_new=np.zeros((m, n)).astype(np.float32)
    
    for i in range(band):
        img_new=img[:,:,i]
        img_range = img_new.max() -img_new.min()
        img_equa[:,:,i]=( img_new - img_new.min()) / img_range 
    return img_equa
...
def save_test_patches(dset_10, dset_20, file, patchSize, border, interp=True):
...
def save_patches_train(dset_20gt, dset_10, dset_20, file, NR_CROP=3721):
    ...
    num=int(np.sqrt(NR_CROP))
    crop=0
    for i in range(0, num):       
        # while True:
        for j in range(0, num):
            upper_left_x =  i * PATCH_SIZE_LR[0]
            upper_left_y =  j * PATCH_SIZE_LR[0]
            ...
            label_20[crop] = np.rollaxis(dset_20gt[crop_point_hr[0]:crop_point_hr[2], crop_point_hr[1]:crop_point_hr[3]], 2)
            image_20[crop] = np.rollaxis(dset_20[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]], 2)
            image_10[crop] = np.rollaxis(dset_10[crop_point_hr[0]:crop_point_hr[2], crop_point_hr[1]:crop_point_hr[3]], 2)
            crop=crop+1
            
    np.save(file + 'data10', image_10)
    image_10_shape = image_10.shape
    del image_10  
    np.save(file + 'data20D', image_20)  
...
def save_patches_Landsat_roi(dset_30gt, dset_10, dset_15, dset_30, outfile):
    PATCH_SIZE_10 = (60, 60)   #SGD=10m
    PATCH_SIZE_30 = (20, 20)  #SGD=30m
    PATCH_SIZE_15 = (40, 40)  #SGD=15m
    # PATCH_SIZE_10 = (180, 180)   #SGD=10m
    # PATCH_SIZE_30 = (60, 60)  #SGD=30m
    # PATCH_SIZE_15 = (120, 120) 
       
    BANDS10 = dset_10.shape[2]
    BANDS15 = dset_15.shape[2]
    BANDS30 = dset_30.shape[2]
    NR_CROP=int((3660/PATCH_SIZE_10[0])*(3660/PATCH_SIZE_10[0]))
    label_30 = np.zeros((NR_CROP, BANDS30) + PATCH_SIZE_10).astype(np.float32)
    image_10 = np.zeros((NR_CROP, BANDS10) + PATCH_SIZE_10).astype(np.float32)
    image_15 = np.zeros((NR_CROP, BANDS15) + PATCH_SIZE_15).astype(np.float32)
    image_30 = np.zeros((NR_CROP, BANDS30) + PATCH_SIZE_30).astype(np.float32)
    
    print(label_30.shape)
    print(image_10.shape)
    print(image_15.shape)
    ...
    for i in range(0, num):       
        # while True:
        for j in range(0, num):
            upper_left_x =  i * PATCH_SIZE_30[0]
            upper_left_y =  j * PATCH_SIZE_30[0]
            crop_point_lr = [upper_left_x,
                             upper_left_y,
                             upper_left_x + PATCH_SIZE_30[0],
                             upper_left_y + PATCH_SIZE_30[1]]
            crop_point_15 = [p*2 for p in crop_point_lr]    
            crop_point_30 = [p*3 for p in crop_point_lr] 
            
            label_30[crop] = np.rollaxis(dset_30gt[crop_point_30[0]:crop_point_30[2], crop_point_30[1]:crop_point_30[3]], 2)
            image_30[crop] = np.rollaxis(dset_30[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]], 2)
            image_10[crop] = np.rollaxis(dset_10[crop_point_30[0]:crop_point_30[2], crop_point_30[1]:crop_point_30[3]], 2)
            image_15[crop] = np.rollaxis(dset_15[crop_point_15[0]:crop_point_15[2], crop_point_15[1]:crop_point_15[3]], 2)
            crop=crop+1
            
    np.save(outfile + 'data10', image_10)
    image_10_shape = image_10.shape
    del image_10
    np.save(outfile + 'data30_gt', label_30)
    del label_30
    
    np.save(outfile + 'data30D', image_30)
    # del image_30
    
    data30_interp = interp_patches(image_30, image_10_shape)
    np.save(outfile + 'data30', data30_interp)
    del data30_interp
    data15_interp = interp_patches(image_15, image_10_shape)
    np.save(outfile + 'data15', data15_interp)
    del data15_interp
    print('Done!')
    
def save_true_patches_Landroi(dset_10, dset_15, dset_30, outfile):
    PATCH_SIZE_10 = (180, 180)   #SGD=10m
    PATCH_SIZE_30 = (60, 60)  #SGD=30m
    PATCH_SIZE_15 = (120, 120) 
    ...
    NR_CROP=int((10980/PATCH_SIZE_10[0])*(10980/PATCH_SIZE_10[0]))
    image_10 = np.zeros((NR_CROP, BANDS10) + PATCH_SIZE_10).astype(np.float32)
    image_30 = np.zeros((NR_CROP, BANDS30) + PATCH_SIZE_30).astype(np.float32)
    image_15 = np.zeros((NR_CROP, BANDS15) + PATCH_SIZE_15).astype(np.float32)
    ...
    np.save(outfile + 'data30_gt', image_30)
    # del image_30
    
    data30_interp = interp_patches(image_30, image_10_shape)
    np.save(outfile + 'data30', data30_interp)
    del image_30, data30_interp
...
def save_patches_Landsat(dset_30gt, dset_15, dset_30, file, size_HR, size_LR):
     
    PATCH_SIZE_HR = (size_HR, size_HR)
    PATCH_SIZE_LR = (size_LR, size_LR)

    BANDS15 = dset_15.shape[2]
    BANDS30 = dset_30.shape[2]
    
    row=int(dset_15.shape[0]/90)
    col=int(dset_15.shape[1]/90)
    NR_CROP = row * col
      
    image_15 = np.zeros((NR_CROP, BANDS15) + PATCH_SIZE_HR).astype(np.float32)   
    crop=0
    for i in range(0, row):       
        # while True:
        for j in range(0, col):                        
            upper_left_x =  i * PATCH_SIZE_HR[0]
            upper_left_y =  j * PATCH_SIZE_HR[0]
            crop_point_hr = [upper_left_x,
                             upper_left_y,
                             upper_left_x + PATCH_SIZE_HR[0],
                             upper_left_y + PATCH_SIZE_HR[1]]              
            image_15[crop] = np.rollaxis(dset_15[crop_point_hr[0]:crop_point_hr[2], crop_point_hr[1]:crop_point_hr[3]], 2)
            crop=crop+1
          
    Effect_index=[]
    for cro in range(0,NR_CROP):
        if np.any(image_15[cro,:,:,:]==0):
            continue      
        else:
            Effect_index.append(cro)
       
    del image_15
    
    label_30 = np.zeros((len(Effect_index), BANDS30) + PATCH_SIZE_HR).astype(np.float32)
    image_30 = np.zeros((len(Effect_index), BANDS30) + PATCH_SIZE_LR).astype(np.float32)
    image_15 = np.zeros((len(Effect_index), BANDS15) + PATCH_SIZE_HR).astype(np.float32)
    ...
    for i in range(0, row):       
        ...
        for j in range(0, col):
            ...
            if crop2 in Effect_index:
                ...
                label_30[crop1] = np.rollaxis(dset_30gt[crop_point_hr[0]:crop_point_hr[2], crop_point_hr[1]:crop_point_hr[3]], 2)
                image_30[crop1] = np.rollaxis(dset_30[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]], 2)
                image_15[crop1] = np.rollaxis(dset_15[crop_point_hr[0]:crop_point_hr[2], crop_point_hr[1]:crop_point_hr[3]], 2)           
                crop1=crop1+1
            crop2=crop2+1
   
    # 归一化
    for i in range(len(Effect_index)):
        image_15[i,:,:,:] = patches_equa(image_15[i,:,:,:])
        label_30[i,:,:,:] = patches_equa(label_30[i,:,:,:])
        image_30[i,:,:,:] = patches_equa(image_30[i,:,:,:])
       
    image_15_shape = image_15.shape
    data30_interp = interp_patches(image_30, image_15_shape)
    
    np.save(file + 'data_hr', image_15) 
    del image_15
    np.save(file + 'data_lr_gt', label_30)
    del label_30
    np.save(file + 'data_lr', data30_interp)
    del data30_interp
...
def save_true_patches( dset_10, dset_20, file, NR_CROP=3721):

    PATCH_SIZE_HR = (180, 180)
    PATCH_SIZE_LR = (90, 90)
    ...
    data20_interp = interp_patches(image_20, image_10_shape)
    np.save(file + 'data20', data20_interp)
    del image_10, data20_interp
    np.save(file + 'data20_gt', image_20)
    del image_20
        
def save_random_patches60(dset_60gt, dset_10, dset_20, dset_60, file, NR_CROP=1764):

    # PATCH_SIZE_10 = (192, 192)
    # PATCH_SIZE_20 = (96, 96)
    # PATCH_SIZE_60 = (32, 32)
    ...
    for i in range(0, num):
        for j in range(0, num):
            ...
            upper_left_x = i * re_pixels
            upper_left_y = j * re_pixels
            crop_point_lr = [upper_left_x,
                             upper_left_y,
            ...
            label_60[crop] = np.rollaxis(dset_60gt[crop_point_hr60[0]:crop_point_hr60[2], crop_point_hr60[1]:crop_point_hr60[3]], 2)
            image_10[crop] = np.rollaxis(dset_10[crop_point_hr60[0]:crop_point_hr60[2], crop_point_hr60[1]:crop_point_hr60[3]], 2)
            image_20[crop] = np.rollaxis(dset_20[crop_point_hr20[0]:crop_point_hr20[2], crop_point_hr20[1]:crop_point_hr20[3]], 2)
            image_60[crop] = np.rollaxis(dset_60[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]], 2)
            crop += 1
    np.save(file + 'data10', image_10)
    image_10_shape = image_10.shape
    del image_10
...
def OpenDataFiles(path, run_60, SCALE, Test = False):
    if run_60:
    ...
    if run_60:
        return Split_Tra_Val_Tes(train_path, [data10, data20, data60], data60_gt, Test)
    else:
        return Split_Tra_Val_Tes(train_path, [data10, data20], data20_gt, Test)  
...
def downPixelAggrLand(img, SCALE=2):
    from scipy import signal
    import skimage.measure
    from scipy.ndimage.filters import gaussian_filter
    ...
    for i in range(0, img.shape[2]):
        img_blur[:, :, i] = gaussian_filter(img[:, :, i], 1/SCALE)
    # New image dims
    new_dims = tuple(s//SCALE+1 for s in img.shape)
    img_lr = np.zeros(new_dims[0:2]+(img.shape[-1],))
    # Iterate through all the image channels with avg pooling (pixel aggregation)
    for i in range(0, img.shape[2]):
        img_lr[:, :, i] = skimage.measure.block_reduce(img_blur[:, :, i], (SCALE, SCALE), np.mean)
...
def recompose_images_sam(a, border, size=None):
    if a.shape[0] == 1:
        images = a[0]
    else:
        # # This is done because we do not mirror the data at the image border
        ...
        # Initialize image
        # print('Image size is: {}'.format(size))
        images = np.zeros((a.shape[3], size[0], size[1])).astype(np.float32)
